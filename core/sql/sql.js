 ...
 * @return {string|{command: number, sql: string, code: number, time: string|number, data: string|Object, message: string}}
 */
module.exports = (sql, connectionIndex) => {
    const dbs = [];

    const dbName = {
        get: () => connectionIndex === -1 ? 'jsdb' : connections[connectionIndex].DBName,
    ...
    const schemaName = {
        get: () => connectionIndex === -1 ? 'public' : connections[connectionIndex].SchemaName,
    ...
    for (let i = 0; i < sql.length; i++) {
        ...
        if (sqlString !== '') {
            ...
            const getPermissions = (dbN = dbName.get()) => {
                let userPrivileges = connectionIndex === -1 ? {"*": 15} : new User(connections[connectionIndex].Username).privileges();
